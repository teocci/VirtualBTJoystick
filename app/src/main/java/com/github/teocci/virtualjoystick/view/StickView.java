...
public class StickView extends View implements Runnable
{
    ...
    public StickView(Context context, AttributeSet attribute, int definitionStyleAttribute)
    {
    ...
    private void initPosition()
    {
        ...
        fixedCenterY = centerY = posY = getHeight() / 2;
...
//        canvas.drawCircle(fixedCenterX, fixedCenterY, borderRadius, paintBackground);
        canvas.drawRect(new Rect(fixedCenterX-(widthSize/2), fixedCenterY-(heightSize/2)+buttonRadius,
                fixedCenterX+(widthSize/2), fixedCenterY+(heightSize/2)-buttonRadius), paintBackground);

        // Draw the circle border
//        canvas.drawCircle(fixedCenterX, fixedCenterY, borderRadius, paintCircleBorder);
        canvas.drawRect(new Rect(fixedCenterX-(widthSize/2), fixedCenterY-(heightSize/2)+buttonRadius,
                fixedCenterX+(widthSize/2), fixedCenterY+(heightSize/2)-buttonRadius), paintCircleBorder);
    ...
    protected void onSizeChanged(int w, int h, int oldW, int oldH)
    {
        ...
        buttonRadius = (int) (h / 3 * RATIO_SIZE_BUTTON);
        borderRadius = (h / 2) - buttonRadius;
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            if (callback != null)
                callback.onHeightMove(posY, fixedCenterY);
        }
...
//                + (posY - centerY) * (posY - centerY));
        ...
        double abs = Math.abs(posY - fixedCenterY);
